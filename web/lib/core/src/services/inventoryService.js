...
async function createInventoryService(db, auditService) {
  async function _getOrCreateInventoryRow(productId, variantId) {
    ...
    if (!row) {
      ...
      row = await db.queryOne(`SELECT * FROM inventory WHERE product_id = ? AND (variant_id IS NOT DISTINCT FROM ?)`, [productId, variantId || null]);
    }
    return row;
  }
  ...
  async function recordStockTransaction({ productId, variantId = null, type, quantity, reason = null,
    ...
    return await db.transaction(async () => {
      ...
      await db.run(`UPDATE inventory SET quantity_on_hand = ?, updated_at = ? WHERE id = ?`,
  ...
  async function reserveStock(productId, variantId, quantity) {
    ...
    await db.run(`UPDATE inventory SET reserved_quantity = reserved_quantity + ?, updated_at = ? WHERE id = ?`,
      [quantity, now(), invRow.id]);
  }

  async function releaseReservedStock(productId, variantId, quantity) {
    await db.run(
      `UPDATE inventory SET reserved_quantity = GREATEST(0, reserved_quantity - ?), updated_at = ?
       WHERE product_id = ? AND (variant_id IS NOT DISTINCT FROM ?)`,
      [quantity, now(), productId, variantId || null]
    );
  }

  async function getStockStatus(productId, variantId = null) {
  ...
  async function transactionHistory(productId, variantId = null, limit = 50) {
    return await db.query(
      `SELECT * FROM stock_transactions WHERE product_id = ? AND (variant_id IS NOT DISTINCT FROM ?)
  ...
  async function listRecentTransactions(limit = 100) {
    return await db.query(
