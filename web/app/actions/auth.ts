...
function credentials(role:Role){\n if(role==='ADMIN') return {user:process.env.ADMIN_USERNAME||'Admin',pass:process.env.ADMIN_PASSWORD};\n return {user:process.env.WORKER_USERNAME,pass:process.env.WORKER_PASSWORD};\n}
...
export async function loginWithCredentials(formData:FormData):Promise<{error?:string;otpRequired?:boolean;devCode?:string}> {
 ...
 const expected=credentials(role); if(!expected.user||!expected.pass) return {error: role==='ADMIN'?'Admin password has not been created yet.':'Worker account must be created by the administrator.'}; if(username!==expected.user||password!==expected.pass) return {error:'Invalid User ID or Password.'};
