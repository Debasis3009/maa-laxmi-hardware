...
export async function createBill(formData: FormData) {
  ...
  for(let i=0;i<5;i++){
    ...
    const product=app.customerBillingService.getProducts(productId).find((p:any)=>p.id===productId) || app.productService.getProduct(productId);
  ...
  revalidatePath('/admin'); revalidatePath('/admin/billing'); revalidatePath('/admin/customers');
