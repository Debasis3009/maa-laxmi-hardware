...
export async function createBill(formData: FormData) {
  ...
  const invoice=await app.customerBillingService.createInvoice({customerId:String(formData.get('customerId')||'')||null,invoiceDate:String(formData.get('invoiceDate')||''),items,paid:Number(formData.get('paid')||0),paymentMethod:String(formData.get('paymentMethod')||'CASH'),paymentReference:String(formData.get('paymentReference')||''),notes:String(formData.get('notes')||'')},await getOwnerId());
